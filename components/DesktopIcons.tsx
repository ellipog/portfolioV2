...
export default function DesktopIcons({
  ...
  const spawnErrorPopups = () => {
    ...
    const spawnError = () => {
      ...
      setErrorPopups((prev) => {
        // Remove old popups if there are too many
        const filtered = prev.slice(-20); // Keep only last 20 popups
        return [
          ...filtered,
          {
      ...
      const nextDelay =
        count <= 3 ? 800 - count * 200 : Math.max(50, 400 - count * 10);
